...
export const useGraphicCardCustomStyle = (props: GraphicCardProps) => {
  ...
  // 解析颜色
  const [audioBgColorClass, audioBgColorStyle] = useComponentColor(
  	toRef(props, 'audioBgColor'),
    'bg'
  )
  ...
  const tagStyle = computed<CSSProperties>(() => {
    ...
    if (!tagBgColorClass.value) {
      style.backgroundColor =
        tagBgColorStyle.value || 'transparent'
    }
  ...
  const audioClass = computed<string>(() => {
    const cls: string[] = []
  
    if (audioBgColorClass.value) cls.push(audioBgColorClass.value)
  ...
  const audioStyle = computed<CSSProperties>(() => {
    ...
    if (!audioBgColorClass.value) {
      style.backgroundColor = audioBgColorStyle.value || 'var(--bee-BG-00-40)'
    }
    return style
  })
