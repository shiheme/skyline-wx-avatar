...
// import bee-icon from '../../icon/src/icon.vue'
  ...
  >
    ...
    <view v-if="showIcon" :class="[ns.e('icon')]">
      ...
      <view v-if="rightIcon" :class="[ns.em('icon', 'custom')]">
        <bee-icon size="20px" :name="rightIcon" />
      </view>
      <!-- 密码显示/隐藏 -->
      <view
      ...
      >
        <bee-icon size="20px" :name="passwordIcon" />
      </view>
      <!-- 清除按钮 -->
      <view
      ...
      >
        <bee-icon size="20px" style="opacity: .3;" name="close-circle-fill" />
      </view>
      <!-- 错误提示图标 -->
      <!-- <view
      ...
      >
        <bee-icon size="24px" :name="validateIcon" />
      </view> -->
    </view>
