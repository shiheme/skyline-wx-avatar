<script lang="ts" setup>
	...
	const { ns, carouselStyle, carouselColorStyle, carouselItemColorStyle, indicatorColorClass, indicatorColorStyle } =
	...
	// for (let i = 0; i < props.listBanner.length; i++) {
	//     // 随机获取图片数据
	...
	//       })
	const rotationimageRef = ref<HTMLElement | null>(null);
	...
	const toBannerBar = (val : number) => {
		// var num:number = props.listBanner.length; 
		// var i:number; 
		// for (i = 0;i<=num;i++) {
		// 	var num2:number = props.listBanner[i].images.length;
		// 	var i2:number; 
		// 	for (i2 = 0;i<=num2;i2++) {
		// 		imagestyle.value[i].images[i2].css = {
		// 			'width': `${props.listBanner[i].images[i2].css_start.width}px`,
		// 			'height':`${props.listBanner[i].images[i2].css_start.height}px`,
		// 		}
		// 	}
			
		// 	// imagestyle.value = "red";
		// 	// indicatorRef.value.style.backgroundColor = "#ffffff";
			
		// 	// 'width': `${cell.css_start.width}px`,
		// 	// 'height':`${cell.css_start.height}px`,
		// 	// 'transform':`translateX(${cell.css_start.x}px) translateY(${cell.css_start.y}px) rotateZ(${cell.css_start.rotateZ}deg) rotateY(${cell.css_start.rotateY}deg) rotateX(${cell.css_start.rotateX}deg)`,
		// 	// 'transform-rigin':`${cell.css_start.originX}% ${cell.css_start.originY}%`,
		// 	// 'opacity':Number(cell.css_start.opacity)
		// }
...
<template>
	<view class="banner-container" :style="carouselStyle">
		<view class="fade-banner">
			<view class="rotation-banner" ref="rotationRef" @click="bannerLink" :style="carouselColorStyle(index === switchIndex)"
				v-for="(item, index) in listBanner" :key="index">
					<image class="banner-img" :src="cell.image" ref="rotationimageRef" v-for="(cell, i) in item.images" :key="i" :style="imagestyle" />
			</view>
		</view>
