...
export const useNavbarCustomStyle = (
  ...
  const [textColorClass, textColorStyle] = useComponentColor(
	...
	const navbarStyle = computed<CSSProperties>(() => {
		...
		} else if (!props.active && !textColorClass.value) {
			style.color = '#FFFFFF'
		} else if (!bgColorClass.value && !textColorClass.value) {
			style.color = 'var(--bee-FC-000)'
		}
	...
	const btnStyle = computed<CSSProperties>(() => {
		...
		// 设置返回按钮的宽高
		style.width = `${navBarBoundingInfo.width / 2}px`
		style.height = `${navBarBoundingInfo.height}px`
	...
	const contentStyle = computed<CSSProperties>(() => {
		...
		if (backButtonType.value !== 'none') {
			style.paddingLeft = `${navBarBoundingInfo.width + navBarBoundingInfo.marginRight
				}px`
		}

		// 判断是否预留右边胶囊安全距离
		if (props.safeAreaInsetRight) {
			style.paddingRight = `${navBarBoundingInfo.width + navBarBoundingInfo.marginRight
				}px`
		}
